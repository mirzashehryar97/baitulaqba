...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!canVoidMatches(currentTeamMember)) {
      throw new ForbiddenError('You do not have permission to void sponsorship matches.');
    }

    const match = await voidSponsorshipMatch(id, body.reason, currentTeamMember.id);
    ...
    return handleMatchApiError(error, 'Could not void sponsorship match.');
  }
}
