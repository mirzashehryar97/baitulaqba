...
const errorMessages: Record<string, string> = {
  ...
  not_allowed: 'This Google account is not linked to an active donor profile yet.',
};

export function DonorLogin({ errorCode }: { errorCode?: string }) {
  ...
  return (
    <main
      ...
      style={{ backgroundImage: "url('/images/login/donor-login-bg.png')" }}
    >
      <div className="absolute inset-0 bg-emerald-deepest/50" />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_27%_42%,rgba(255,248,230,0.14),transparent_32%),linear-gradient(90deg,rgba(8,34,31,0.66),rgba(8,34,31,0.16))]" />

      <div className="relative z-10 flex w-full max-w-6xl justify-center md:justify-start">
        <div className="w-full max-w-md rounded-2xl border border-gold/25 bg-offwhite/95 p-7 shadow-card backdrop-blur-sm">
          ...
          <p className="mt-2 text-sm leading-relaxed text-ink/65">
            Sign in with the Google account connected to your donor profile.
          </p>

          <GoogleSignInButton href="/auth/login?next=/portal" />
          ...
          <p className="mt-5 text-xs font-semibold leading-relaxed text-ink/45">
            Donor access is available after the team creates your donor profile.
          </p>
        </div>
      </div>
    </main>
